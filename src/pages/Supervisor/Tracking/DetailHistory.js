...
function DetailHistorySV(props) {
     ...
     const getCheckout = () => {
          ...
          axios.get(`/supervisor/tracking-sales/detail/${checkout_id}`, {
               headers: {
                         ...
                         <div className="w-full px-3 py-3 justify-between rounded-lg h-auto flex">
                              <div className="flex">
                                   <img
                                        ...
                                        src={`${process.env.REACT_APP_HOST_HEROKU}${listProduct?.length > 0 ? listProduct[0]?.checkout?.apotik?.image : ''}`}
                                        alt="img"
                                   />
                                   <div className="ml-3">
                                        <p className="font-bold text-gray-600">{listProduct.length > 0 ? listProduct[0].checkout.apotik.name : ''}</p>
                                        <p className="text-xs text-gray-600">{listProduct.length > 0 ? listProduct[0].checkout.apotik.address : ''}</p>
                                   </div>
                              </div>
                         </div>
                    </div>
