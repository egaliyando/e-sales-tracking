          ...
          <Container>
               ...
               <div className="px-3 pt-20">
                    <div className="h-64 w-full rounded-lg flex flex-col justify-center bg-white">
                         <img className="self-center mt-5 h-24 w-25 rounded-full" src={`${process.env.REACT_APP_HOST_HEROKU}${image}`} alt="img" />
                         <div className="mb-5 mt-3">
                              <p className="text-2xl text-gray-600 text-center font-bold">{name}</p>
                              <p className="text-center text-gray-600">{nik}</p>
                         </div>
                    </div>
                    <div className="grid grid-cols-2  bg-white rounded-lg p-4 mt-3">
                         <div>
                              <p className="font-bold  text-gray-600">Address </p>
                              <p className="font-bold  text-gray-600 mt-2">Birthday </p>
                              <p className="font-bold  text-gray-600 mt-2">Role </p>
                         </div>

                         <div>
                              <p className="text-gray-600">{address}</p>
                              <p className="text-gray-600 mt-2">{formatBirthday}</p>
